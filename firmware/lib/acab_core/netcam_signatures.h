 ...
 *
 * WYZE IS DELIBERATELY EXCLUDED: Wyze cams ride the shared Espressif OUI, and Wyze's own
 * OUIs also cover plugs / bulbs / locks, so a Wyze match identifies nothing camera-specific.
 */
...
// Registry-confirmed IP-camera brand OUIs (IEEE MA-L). Verified 2026-07-17 via the live
...
// correctly AND the device really was a camera. The other 15 OUIs in this table remain
...
// four entries flagged validated=1 get it; the other 15 stay registry-only at 65.
